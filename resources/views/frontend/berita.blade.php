...
<div class="container-fluid bg-primary mb-2">
  <div class="d-flex flex-column align-items-center justify-content-center" style="min-height: 100px">
    <h3 class="display-4 font-weight-bold text-white">Berita</h3>
...
<div class="container-fluid pt-5">
  <div class="container">
    <div class="row">
      <div class="col-12 text-center mb-2">
        <ul class="list-inline kategori-filter mb-4" id="portfolio-flters">
          <a class="btn btn-outline-primary m-1 {{ ($kategori == 'Semua')?" active":"" }}" href="{{ route('berita') }}"">Semua</a>
          @foreach ($kategori_list as $item)
          @if ($kategori == $item->nama_kategori)
            <a class=" btn btn-outline-primary m-1 active"
            href="{{ route('berita') }}?kategori={{ urlencode($item->nama_kategori) }}">{{
            $item->nama_kategori }}</a>
          @else
          <a class="btn btn-outline-primary m-1" href=" {{ route('berita') }}?kategori={{ urlencode($item->nama_kategori) }}">{{
            $item->nama_kategori }}</a>
          @endif
          @endforeach
        </ul>
    ...
    @if(count($berita_list)<1) <div class="row">
    ...
    <div class="col-lg-4 mb-5">
      <div class="card border-0 bg-light shadow-sm pb-2">
        ...
        <div class="card-body text-center">
          <a href="{{route('berita.detail',$p->slug)}}">
            <h5>{{ $p->judul }}</h5>
          </a>
...
<div class="col-md-12 mb-4">
  <nav aria-label="Page navigation">
    <ul class="pagination justify-content-center mb-0">
      @for ($i = 1; $i <= $berita_list->lastPage(); $i++)
        @if ($i == $berita_list->currentPage())
